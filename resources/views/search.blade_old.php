...
//    $ = jQuery;
    ...
    $(function () {
        ...
        $("#ztpl-addtocart").on('click', function () {
            ...
            var out_of_stock = '<?php echo $shop_find['allow_out_of_stock_products_to_order'] ?>';
        ...
        $(".ztpl-addrow").on('click', function () {
            ...
            var new_add = '<div class="ztpl-bo-line-item ">' +
                    ...
                    '<div class="ztpl-bo-variant-input-section ztpl-col-sm-5 ztpl-col-lg-5 ztpl-col-md-5 ztpl-col-xs-12 ztpl-ui-widget">' +
                    '<input name="search_text" type="text" placeholder="Search Product(by name or SKU)" maxlength="524288" class="ztpl-bo-variant-input ztpl-search_text" />' +
                    '</div>' +
                    '<div class="ztpl-col-lg-2 ztpl-col-md-2 ztpl-col-sm-2 ztpl-col-xs-4 ztpl-qt_box">' + '<input type="text" class="ztpl-product_quantity" disabled="disabled" placeholder="Quantity"/>' +
                    '</div>' +
                    '<div class="ztpl-col-lg-2 ztpl-col-md-2 ztpl-col-sm-2 ztpl-col-xs-4 ztpl-cost_box">' + '<input type="text" class="ztpl-product_cost" disabled="disabled" placeholder="Cost" />' +
                    '</div>' +
                    '<div class="ztpl-col-lg-2 ztpl-col-md-2 ztpl-col-sm-2 ztpl-col-xs-4 ztpl-total_box">' + '<input type="text" class="ztpl-total" disabled="disabled" placeholder="Total" />' +
                    '</div>' +
        ...
        function run_auto_complete()
        {
            $(".ztpl-search_text").autocomplete({
                source: function (request, response) {
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            response($.map(data.results, function (results) {
                ...
                select: function (event, ui) {
                    ...
                    if (ui.item.stock == 0 && out_of_stock == 0)
                    {
                    ...
                    $(this).parent().parent().find('.ztpl-varient').attr("stock", ui.item.stock);
                    $(this).parent().parent().find('.ztpl-varient').attr("cost", ui.item.cost);
                    $(this).parent().next().next().find('.ztpl-product_cost').val(formated_cost);
                    $(this).parent().next().find('.ztpl-product_quantity').removeAttr('disabled');
                    $(this).parent().next().find('.ztpl-product_quantity').attr("max", ui.item.stock);
                    ...
                    if (shopify_inventory == "shopify")
                    {
                        if (ui.item.stock == 0)
                        {
                            ...
                            {
                                $("<strong class='ztpl-qtywarning' style='color:red;display:none';>OUT OF STOCK:" + ui.item.stock + "</strong>").insertAfter(qtypath);
                            } else {
                            ...
                            var quantityypath = $(this).parent().next().find('.ztpl-product_quantity');
                            quantityypath.val(1);
                            var quantity_display = $(this).parent().next().find('.ztpl-qtywarning');
                            if (available_quantity_status == 1)
                            {
                                if ($(quantity_display).length <= 0)
                                {
                                    $("<strong class='ztpl-qtywarning' style='color:green;display:<?php echo ($shop_find['show_available_quantity'] == 1) ? 'block' : 'none'; ?>'><?php echo ($shop_find['available_quantity_label'] != '') ? $shop_find['available_quantity_label'] : 'Available Quantity'; ?>:" + ui.item.stock + "</strong>").insertAfter(quantityypath);
                                } else {
                                    $(quantity_display).text('<?php echo ($shop_find['available_quantity_label'] != '') ? $shop_find['available_quantity_label'] : 'Available Quantity'; ?>:' + ui.item.stock);
                                    $(quantity_display).css('color', 'green');
                                }
                            }
                        }

                        $(this).parent().next().next().next().find('.ztpl-total').val(formated_cost);
                    } else {
                        var quantityypath = $(this).parent().next().find('.ztpl-product_quantity');
...

                        {
                            ...
                            if (qty > parseInt(stock) && (parseInt(out_of_stock) == 0)) {
                                ...
                                if (qty >= 0 && parseInt(stock) != 0) {
                                    ...
                                    $(this).parent().next().find('.ztpl-product_quantity').attr("max", stock);
                                    ...
                                    $(this).parent().next().next().find('.ztpl-total').val(0);
...

                    {
                        ...
                        } else
                        {
                            if (product_image_status == 1)
                            {
                                ...
                                img = "<a><img src='" + item.productimage + "' height='32' width='32'/><span>" + item.label + "</span></a>";
...
<div class="ztpl-bulk-order-wrapper">
    ...
    <div class="ztpl-bo-app">
        ...
        <div class="ztpl-row main-label">

            <div class="ztpl-col-sm-12 ztpl-col-md-5 ztpl-col-lg-5 ztpl-col-xs-12 ztpl-search-label"><?php echo ($shop_find['product_name_label'] != '') ? $shop_find['product_name_label'] : 'Search Product(by name or SKU)' ?></div>

            <div class="ztpl-col-sm-12 ztpl-col-md-2 ztpl-col-lg-2 ztpl-col-xs-4 ztpl-child-label"><?php echo ($shop_find['quantity_label'] != '') ? $shop_find['quantity_label'] : 'Quantity' ?></div>

            <div class="ztpl-col-sm-12 ztpl-col-md-2 ztpl-col-lg-2 ztpl-col-xs-4 ztpl-child-label"><?php echo ($shop_find['cost_label'] != '') ? $shop_find['cost_label'] : 'Cost' ?></div>

            <div class="ztpl-col-sm-12 ztpl-col-md-2 ztpl-col-lg-2 ztpl-col-xs-4 ztpl-child-label"><?php echo ($shop_find['total_label'] != '') ? $shop_find['total_label'] : 'Total' ?></div>
        ...
        <div class="ztpl-row ztpl-mainrow">
            ...
            <div class="ztpl-bo-line-item">
                ...
                <div class="ztpl-row-wrapper"><input name="varient" type="hidden" class="ztpl-varient" />

                    <div class="ztpl-bo-variant-input-section ztpl-col-sm-5 ztpl-col-lg-5 ztpl-col-md-5 ztpl-col-xs-12 ztpl-ui-widget"><input name="search_text" type="text" placeholder="<?php echo ($shop_find['product_name_label'] != '') ? $shop_find['product_name_label'] : 'Search Product(by name or SKU)' ?>
					"maxlength="524288" class="ztpl-bo-variant-input ztpl-search_text" /></div>

                    <div class="ztpl-col-lg-2 ztpl-col-md-2 ztpl-col-sm-2 ztpl-col-xs-4 ztpl-qt_box"><input type="text" class="ztpl-product_quantity" min="1" disabled="disabled" placeholder="<?php echo ($shop_find['quantity_label'] != '') ? $shop_find['quantity_label'] : 'Quantity' ?>" /></div>
        ...
        <div class="row">
            <div class="col-md-5">
                <a class="ztpl-addrow"><span class="ztpl-glyphicon ztpl-glyphicon-plus ztpl-plus-icon"></span><b>Add Product</b></a>
            </div>
        ...
        <div class="ztpl-bo-add-line-item ztpl-row-wrapper">
            <div>
                <div class="ztpl-bo-cart-controls">
                    <div class="ztpl-bo-cart-link ztpl-col-sm-6 ztpl-col-xs-12"><button class="ztpl-btn ztpl-btn-primary" onclick="document.location.href = '/cart'"> Go To Cart</button><button class="ztpl-btn ztpl-bo-clear-cart ztpl-btn-primary" id="ztpl-empty_cart">Clear Cart</div>

                    <div class="ztpl-col-sm-6 ztpl-col-xs-12 ztpl-bo-cart-link"><button class="ztpl-btn ztpl-bo-update-cart ztpl-btn-primary" id="ztpl-addtocart" disabled="disabled"> Add To Cart </button><button type="reset" value="Reset" class="ztpl-btn ztpl-btn-primary" id="ztpl-reset_all">Reset Form</button></div>
