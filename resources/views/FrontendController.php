            ...
            <tr>
                ...
                <td>
                    ...
                    if ($row_variant->inventory_quantity == 0) {
                        ...
                        echo '<input type="text" class="quntity-input" id="qty_outer_' . $row_variant->id . '" value="1" min="1" />';
                    }
                    ?>                    
                </div>
            </td>
...
<!--         <script>
    ...
    public function get_all_product1(Request $request){
        ...
        if($search){
            $products = $sh->callAdvance(['URL' => '/admin/api/' . $this->apiVersion . '/products.json?limit=' . $limit . '&title='.urlencode($search), 'METHOD' => 'GET']);
